...
|IchigoJamフォント挿入|		{a文字コード(0～255)}|
|登録メッセージの挿入|		{mメッセージ番号(0～7)}|
|変数値の挿入|				{v変数番号(0～9)}|
|半角2文字の挿入|				{h文字1文字2(文字は0x20～0x7dの範囲の文字)}|
|ドットの表示|				{横座標(0～7)縦座標(0～7)ドット(0or1)} |
|表示の消去|					{}|
...
|機能名|コマンド|
|:--|:--|
|スクロール動作の設定|		{s+\|-\|動作コード(0～15 デフォルトは5)}|
|スクロール速度の設定|		{t速度(0～32767msec デフォルトは 60)}|
|即時ウェイト|				{dウエイト時間(0～32767msec)}|
|1ドットスクロール|			{>[動作コード(0～15 デフォルトは5)]}|
|メッセージ間ウエイト設定|	{w間隔(0～32767 msec デフォルトは0)}|
|文字間ウェイト設定|			{c間隔(0～32767 msec デフォルトは0)}|
|文字の回転補正|				{r+\|-\|向き(0～3 デフォルトは3)}|
|コード系をIchigoJamに設定|	{i}|
|utf15コード指定|				{n}|
|設定値をデフォルトに戻す|	{!}|
